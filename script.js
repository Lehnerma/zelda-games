...
//gameCards rendern
function rednerGameCard() {
  let cardsRef = document.getElementById('game_cards');
  cardsRef.innerHTML = '';
  for (let gameIndex = 0; gameIndex < zeldaGames.length; gameIndex++) {
  cardsRef.innerHTML += getGameCardTemplet(gameIndex);
  }
}
console.log(zeldaGames[1].comments);

//rederComments
function renderComment(gameIndex) {
  let commentRef = document.getElementById("comments_container");
  commentRef = '';
